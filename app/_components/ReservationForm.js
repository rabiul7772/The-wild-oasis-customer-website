...
import { fixDateForSupabase } from '../_lib/data-service';
import { useReservation } from './ReservationContext';

function ReservationForm({ cabin, user }) {
  const { range, resetRange } = useReservation();

  const startDate = fixDateForSupabase(range?.from);
  const endDate = fixDateForSupabase(range?.to);

  const numNights = differenceInDays(endDate, startDate);

  const { regularPrice, discount, id, maxCapacity } = cabin;
    ...
    <div className="scale-[1.01]">
      ...
      >
        ...
        <div className="flex justify-end items-center gap-6">
          ...
          {startDate && endDate && <SubmitButton />}
        </div>
      </form>
    </div>
  );
